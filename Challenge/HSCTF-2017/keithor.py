from hashlib import *
from base64 import *

#hint: flag only contains capital letters and numbers and no special characters
FLAG = b'TlZMN09B'
FLAG = b64decode(FLAG)
b64hash = b'':
def main():
    n = ''
    n += md5(FLAG).hexdigest()
    n += sha1(FLAG).hexdigest()
    n += sha224(FLAG).hexdigest()
    n += sha256(FLAG).hexdigest()
    n += sha384(FLAG).hexdigest()
    n += sha512(FLAG).hexdigest()

    if b64encode(n.encode()) == b64encode 
        print('Correct flag!')
    else:
        print('Incorrect flag :(')


def get_hash():
    global b64hash
    sumhash = b64decode(b64hash).decode()
    md5hash = sumhash[:32]
    sha1hash = sumhash[32:72]
    sha224hash = sumhash[72:118]
    sha256hash = sumhash[128:192]


    print md5hash
    print sha1hash # f4fdaef8ade8edf707858fe4294d780d69d4d6a8 => search hash database
    print sha224hash
    
get_hash()
